...
export default function PageDev() {
  ...
  const fetchFeedbacks = async () => {
    try {
      const res = await fetch(`https://animosity-web-g5ao.vercel.app/feedbacks`);
  ...
  const fetchAtualizacoes = async () => {
    try {
      const res = await fetch("https://animosity-web-g5ao.vercel.app/api/atualizacoes");
      const data = await res.json();
  ...
  const handleDeleteFeedback = async (id_usuario, versao, mensagem) => {
    try {
      const res = await fetch(`https://animosity-web-g5ao.vercel.app/api/feedback/${id_usuario}?versao=${versao}&mensagem=${encodeURIComponent(mensagem)}`, {
        method: 'DELETE'
      });
  ...
  const handleSaveAtualizacao = async (id) => {
    try {
      const res = await fetch(`https://animosity-web-g5ao.vercel.app/api/atualizacoes/${id}`, {
        method: "PUT",
...
const handleEnviarJogo = async () =>{
  try{
    const res = await fetch("https://animosity-web-g5ao.vercel.app/api/downloads", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(form)
    });
